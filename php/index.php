...
{
    $url = "http://" . JAVA_HOST . ":" . JAVA_PORT . $data['path'];
    $decodedBody = base64_decode($data['body']);

    $ch = curl_init($url);

    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, $data['method']);
    if ($data['method'] === 'POST' || $data['method'] === 'PUT' || $data['method'] === 'PATCH') {
        curl_setopt($ch, CURLOPT_POSTFIELDS, $decodedBody);
    }
    ...
    if ($responseBody === false) {
        return json_encode([
            "status" => "ERROR",
            "message" => "Cannot connect to Java server at " . JAVA_HOST . ":" . JAVA_PORT,
            "debug_error" => $error
        ]);
    }
